...
def allTraversed():
    if len(paths) < len(roomGraph):
        return False
    return True


def reverseExit(p_exit):
    if p_exit == 'n':
        return 's'
    if p_exit == 's':
        return 'n'
...
def dfs_path(starting_room):

    player.currentRoom = starting_room
    paths[player.currentRoom.id] = {
        i: '?' for i in player.currentRoom.getExits()}
    s = Stack()
    s.push(player.currentRoom.id)
    visited_exits = {k: set() for k in range(len(roomGraph))}

    usedExit = None
    while s.size() > 0:
        s.pop()
        exits = player.currentRoom.getExits()
        prevId = player.currentRoom.id

        if usedExit is not None:
            visited_exits[player.currentRoom.id].add(reverseExit(usedExit))

        if len(exits) == 1:
            traversalPath.append(exits[0])
            player.travel(exits[0])
            s.push(player.currentRoom.id)
            usedExit = exits[0]
        else:
            for i in exits:
                if i in visited_exits[player.currentRoom.id]:
                    exits_copy = {item[0]: item[1]
                                  for item in paths[player.currentRoom.id].items()}
                    for j in visited_exits[player.currentRoom.id]:
                        exits_copy.pop(j)
                    if '?' in exits_copy.values():
                        continue
                if paths[player.currentRoom.id][i] == '?':
        ...
        if usedExit is not None:
            paths[prevId][usedExit] = player.currentRoom.id
            if player.currentRoom.id not in paths.keys():
                paths[player.currentRoom.id] = {
                    i: '?' for i in player.currentRoom.getExits()}
        else:
            break
        ...
        for v in roomGraph[path[-1]][1].values():
...
dfs_path(world.startingRoom)
print(paths)
print(traversalPath)


""" for i in range(len(roomGraph)):
    print(f"{i}: {roomGraph[i]}") """
